...
export const adminAPI = {
  async getAllUsersWithStats() {
    ...
    const enriched = await Promise.all((users as any[]).map(async (u) => {
      const reqs = await convex.query('serviceRequests:getByUserId', { userId: u.id });
      const totalRevenue = (reqs as any[]).reduce((sum, r) => sum + (r.payment_completed ? (r.total_cost ?? 0) : (r.deposit_paid ?? 0)), 0);
      return {
        ...u,
        ticketCount: (reqs as any[]).length,
        totalRevenue,
        pendingTickets: (reqs as any[]).filter(r => r.status === 'Pending').length,
        completedTickets: (reqs as any[]).filter(r => r.status === 'Completed').length,
        lastActivityDate: (reqs as any[]).length > 0 ? (reqs as any[])[0].created_at : null,
      };
    }));
  ...
  async getAllServiceRequests(limit = 100, offset = 0) {
    ...
    return getOrFetch<{ requests: unknown[]; total: number }>(key, async () => {
      ...
      const sorted = (all as any[]).sort((a, b) => (b.created_at as string).localeCompare(a.created_at));
  ...
  async getRequestsByStatus(status: string, limit = 100, offset = 0) {
    ...
    return getOrFetch<{ requests: unknown[]; total: number }>(key, async () => {
      const all = await convex.query('admin:listRequests', {});
      const filtered = (all as any[]).filter(r => r.status === status).sort((a, b) => (b.created_at as string).localeCompare(a.created_at));
  ...
  async getGlobalStats() {
    return getOrFetch<{ totalUsers: number; totalTickets: number; pendingTickets: number; completedTickets: number; inProgressTickets: number; totalRevenue: number }>('admin:globalStats', async () => {
      ...
      return {
        ...
        totalTickets: (reqs as any[]).length,
        pendingTickets: (reqs as any[]).filter(r => r.status === 'Pending').length,
        completedTickets: (reqs as any[]).filter(r => r.status === 'Completed').length,
        inProgressTickets: (reqs as any[]).filter(r => r.status === 'In-Progress').length,
        totalRevenue,
      };
    });
  },
  ...
  async searchRequests(query: string, limit = 50, offset = 0) {
    ...
    return getOrFetch<{ requests: unknown[]; total: number }>(key, async () => {
      ...
      ).sort((a, b) => (b.created_at as string).localeCompare(a.created_at));
      return { requests: filtered.slice(offset, offset + limit), total: filtered.length };
    });
  },
